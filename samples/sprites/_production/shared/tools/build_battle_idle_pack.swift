...
guard CommandLine.arguments.count == 8 || CommandLine.arguments.count == 9 else {
    fputs("usage: build_battle_idle_pack.swift source.png canonical_idle.png output_dir asset_stem animation_name res_directory delay1,...,delay28 [none|reverse-settle]\n", stderr)
    exit(2)
}
...
let repairMode = CommandLine.arguments.count == 9 ? CommandLine.arguments[8] : "none"
...
if repairMode == "reverse-settle" {
    // Some generated 7x4 atlases clip the final row against the image boundary.
    // Reuse intact poses in reverse progression so the idle settles naturally
    // instead of shrinking at the row transition.
    let recoverySources = [19, 18, 17, 14, 10, 4, 1]
    for (offset, sourceIndex) in recoverySources.enumerated() {
        frames[20 + offset] = frames[sourceIndex]
    }
}
frames[0] = canonical
...
let repairNote = repairMode == "reverse-settle"
    ? ", \"Frames 20–26 reuse intact earlier poses as a reverse settle because the generated atlas was clipped at its right and bottom boundaries.\""
    : ""
let profile = """
{
  "schema_version": 1,
  ...
  "notes": ["Frames 0 and 27 are pixel-identical canonical idle frames.", "Visual idle only; no gameplay event hooks."\(repairNote)]
}
"""
