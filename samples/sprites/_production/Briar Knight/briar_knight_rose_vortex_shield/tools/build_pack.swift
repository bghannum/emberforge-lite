...
func compositeCanonicalKnight(_ frame: inout [UInt8], canonical: [UInt8]) {
    for pixel in 0..<(cellSize * cellSize) where canonical[pixel * 4 + 3] >= 8 {
        let offset = pixel * 4
        frame[offset] = canonical[offset]
        frame[offset + 1] = canonical[offset + 1]
        frame[offset + 2] = canonical[offset + 2]
        frame[offset + 3] = canonical[offset + 3]
    }
}
...
for index in 1..<(frameCount - 1) { compositeCanonicalKnight(&frames[index], canonical: canonicalIdle) }
