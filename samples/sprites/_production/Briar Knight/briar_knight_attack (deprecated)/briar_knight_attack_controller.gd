class_name BriarKnightAttackController
extends Node2D
...
const ATTACK_ANIMATION := &"lunge_attack"
const HIT_START_FRAME := 8
const HIT_END_FRAME := 9
const DAMAGE_EVENT_FRAME := 9
const LUNGE_OFFSETS := PackedFloat32Array([
	0.0, 0.0, -2.0, -4.0, -6.0, 0.0, 14.0, 32.0,
	56.0, 82.0, 94.0, 88.0, 64.0, 38.0, 16.0, 0.0,
])
...
func _on_frame_changed() -> void:
	if attack_sprite.animation != ATTACK_ANIMATION:
		return
	var frame_index := clampi(attack_sprite.frame, 0, LUNGE_OFFSETS.size() - 1)
	attack_sprite.position.x = LUNGE_OFFSETS[frame_index]
