...
let targetRootX = 157
let targetBaselineY = 307
let targetSpriteHeight = 256
let targetSpriteMaxWidth = 310
...
let canonicalIdle = readExactFrame(path: canonicalIdlePath)
frames[0] = canonicalIdle

// The generated penultimate source cell omitted the caster. Restore the idle
// silhouette beneath the remaining dispersing mist instead of letting him vanish.
for pixel in 0..<(cellSize * cellSize) where canonicalIdle[pixel * 4 + 3] >= 8 {
    let offset = pixel * 4
    frames[14][offset] = canonicalIdle[offset]
    frames[14][offset + 1] = canonicalIdle[offset + 1]
    frames[14][offset + 2] = canonicalIdle[offset + 2]
    frames[14][offset + 3] = canonicalIdle[offset + 3]
}
