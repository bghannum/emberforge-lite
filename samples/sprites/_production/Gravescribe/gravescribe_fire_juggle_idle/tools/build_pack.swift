...
guard CommandLine.arguments.count == 9 else {
    fputs("usage: build_pack.swift source-atlas.png output-directory asset-stem preserve|match-first start-idle-name end-idle-name canonical-idle.png delay1,...,delay20\n", stderr)
    exit(2)
}
...
guard frameDelays.count == frameCount else {
    fputs("exactly 20 comma-separated frame delays are required\n", stderr)
    exit(2)
}
...
guard
    ...
    fputs("source atlas must be 1570x1256 RGBA\n", stderr)
    exit(1)
}
...
try writePNG(frames[frameCount - 1], width: cellSize, height: cellSize,
...
try writePNG(alignedAtlas, width: atlasWidth, height: atlasHeight,
             to: outputDirectory.appendingPathComponent("\(assetStem)_20f_aligned.png"))
